<template>
    ...
    <div class="wrapper fadeInDown tam">
        <div id="formContent">
            ...
            <input type="button" class="fadeIn first" value="Consultar" v-on:click="usuarioPC(user)" >
            <div v-if="loading" >
...
export default {
    ...
    methods: {
        ...
        async  usuarioPC(user) {
            this.loading = true;
            this.datos = 'Consultando datos Espere por favor'
            let self = this
            try {
            const result = await axios.get(`http://76.252.93.168:3000/api/${user}`)
